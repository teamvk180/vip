...
int main(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; ++i)
    {
        if (std::strstr(argv[i], "="))
        {
            ...
            ++filename_;
            if (std::strstr(argv[i], "of=") || std::strstr(argv[i], "outfile="))
            {
                output_file = filename_;
            }
            else if (std::strstr(argv[i], "if=") || std::strstr(argv[i], "infile="))
            {
    ...
    else
    {
        ...
        {
            std::cout << std::endl
                      << "(q, quit, or exit) || Enter arithmetic expression: "; // add exit options
            if (!std::getline(std::cin, expression_line))
                return -1; // I/O error
            if (expression_line == "q" || expression_line == "quit" || expression_line == "exit")
                break;
